...
> **产品名**：**luluk**
> **状态**：阶段 0 原型已验证，部分品牌化决策完成，待补全（图标/域名/Bundle ID）后启动 Plan B
> **更新**：2026-05-02
...
### 一句话定位
**luluk 是为看外语视频的普通观众做的"AI 自动字幕播放器"**——打开视频，约 11 秒内看到目标语言字幕，全程零配置。
...
| **首字幕等待** | 取决于网络/搜索结果 | **30 秒**（流水线第一段完成即可看）|
| **字幕翻译** | 不提供 | **三档翻译可选** |
...
**本产品用户旅程**：
1. 打开视频
2. **30 秒后**第一行字幕出现（自动生成 + 翻译 + 双语显示）
3. 全程播放，字幕持续追加生成

**这是 18 倍体验差距**（实测：22 分钟视频，10 分钟 → 33 秒）。
...
本产品:  Swift App ──→ libmpv ──→ 视频解码渲染
                              ...
                              .srt 文件
                              ↑
         AISubtitleService ────┘（FSEventStream watch + auto sub-reload）
              │
              ├─→ ffmpeg 切片
              ├─→ whisper-cli (CoreML 加速) × 并发
              ├─→ TranslationProvider（DeepSeek/MiniMax/NLLB/我们自家）
              └─→ srt 拼接 + 增量写入
```
...
- [ ] **磁盘 watch + 自动重载**：FSEventStream 监听 SRT 改动，自动调 `mpv sub-reload`。**默认开**，UI 设置项「字幕变更时自动刷新」可关
- [ ] **幻觉清理**：基于原型 `pipeline/sanitize.py`，扩展模式覆盖
- [ ] **whisper 模型管理**：首次启动**应用内一键下载**（默认 turbo ~1.5GB；large-v3 + CoreML 是可选增强 ~4GB）
- [ ] **API key 配置**：用户填自己的 DeepSeek/MiniMax key
- [ ] **本地翻译兜底**：**NLLB-200-distilled-600M**（零配置，质量稍差）
- [ ] **双语字幕**：可选开关，原文上译文下
- [ ] **字幕样式预设**：电影标准样式默认开
...
├── AISubtitleService.swift       ← 主调度，参考原型 produce.py
├── AISubtitle/
│   ├── AudioSplitter.swift       ← ffmpeg 静音切片，参考 audio_split.py
│   ├── WhisperRunner.swift       ← spawn whisper-cli + CoreML，参考 transcribe.py
│   ├── TranslationProvider.swift ← 协议（参考 §15）
│   ├── Providers/
│   │   ├── DeepSeekProvider.swift    ← OpenAI 兼容，用户自填 key
│   │   ├── MiniMaxProvider.swift     ← OpenAI 兼容，用户自填 key
│   │   ├── OpenAIProvider.swift      ← 用户自填 sk-xxx
│   │   ├── CustomProvider.swift      ← 用户填任意 OpenAI 兼容 endpoint
│   │   ├── LulukCloudProvider.swift  ← endpoint 写死 api.luluk.xyz（前端开源，后端闭源，见 §15）
│   │   └── NLLBLocalProvider.swift   ← 调本地 NLLB helper
│   ├── Sanitizer.swift           ← 幻觉清理，参考 sanitize.py
│   └── SRTMerger.swift           ← 偏移合并 + 双语
├── SubtitleFileWatcher.swift     ← FSEventStream 实现自动重载
├── AISubtitlePrefViewController.swift   ← Provider 选择 UI（见 §15.2）
├── LulukCloudAuth.swift          ← luluk 用户登录/订阅状态查询（前端组件）
└── ModelDownloader.swift         ← 首次启动引导下载
```
        ...
        TranslationProvider  TranslationProvider  TranslationProvider
              ...
              └───────────────────┼───────────────────┘
                        ...
                        写视频同目录 .zh.srt / .bilingual.srt
                                  ↓
                        FSEventStream 触发 mpv sub-reload
                                  ↓
                            用户屏幕字幕更新
```
...
#### 文件 watch
- macOS FSEventStream 监听视频同目录
- `.zh.srt` 文件改动 → 调 PlayerCore.reloadAllSubs()
- IINA 已有 `subReload` MPVCommand，直接复用
...
| 类别 | Provider | 面向用户 | 质量 | 速度 | 用户操作 | 成本 |
|------|---------|---------|------|------|---------|------|
| **luluk Cloud** | LulukCloudProvider（接 api.luluk.xyz）| 普通观众首选 | 高（含我们调优 prompt + 缓存）| 快 | 注册 + 订阅 | 月/年/按量套餐 |
| **用户自带 key** | DeepSeek / MiniMax / OpenAI / Custom | 极客 | 高 | DeepSeek ~50s/200行 | 填 API key | 自付上游 ¥0.001-0.003/分钟 |
| **本地兜底** | NLLBLocalProvider（distilled-600M）| 全员零配置 / 离线 | 中 | M4 ~120s/200行 | 无 | 免费 |

**为什么默认要本地 NLLB**：目标用户是普通观众，**默认必须零配置可用**，否则用户在配置页就流失。NLLB-200-distilled-600M 翻译质量虽然不如 DeepSeek，但"看懂剧情"够用，且 200 种语言互译开箱即用。luluk Cloud 试用余额耗尽后也回落到 NLLB（避免硬阻塞）。
...
**V1 baseline（M4 + 24GB + turbo + GGML+Metal）**：
- 实时倍速 **10×**（与视频长度无关）
- 首字幕延迟 **~11 秒**（与视频长度无关）
...
| **Logo** | `assets/logo.png`（405×425, RGBA）已就位 |
...
| 首字幕 UX | **自动 watch 默认开**（FSEventStream 监听 SRT 文件，自动调 sub-reload），UI 设置项可关 |
...
| Logo | `assets/logo.png` 已就位（405×425, RGBA）|
| 主色 | 绿色（深翠绿主 + 浅绿辅，从 logo 提取，精确 hex 在 fork 时定）|

### 🚀 启动状态：决策已齐全，可以开始 fork

剩余仅**执行项**（见 §9）：
- [ ] 从 logo.png 生成 macOS .icns 全尺寸（16/32/64/128/256/512/1024 + @2x）
- [ ] 取主色精确 hex
- [ ] 注册 Apple Developer Program
- [ ] 配置 luluk.xyz DNS（主站 + 子域名）
- [ ] 创建 ayaoplus/luluk GitHub 仓库
