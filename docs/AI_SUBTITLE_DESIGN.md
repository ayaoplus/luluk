...
│   │   }                                              │
│   │     ↓ 段级并发上限 1                              │
│   │       (SPEC 原写 3；M3 实测 3 并发 whisper-cli    │
│   │        与 mpv 抢 GPU 致破音，回退到 1)            │
│   │                                                  │
│   ├─→ WhisperRunner.transcribe(segment) async throws │
│   │     spawns whisper-cli (Metal · GGML，无 CoreML) │
│   │     → TranscriptionResult                        │
│   │                                                  │
...
└──────────────────────────────────────────────────────┘
    │
    ↓ mtime 轮询 500ms（M3 简化实现，FSEventStream 留 M5）
┌───┴──────────────────────────────────────────────────┐
│ SubtitleFileWatcher  (per-PlayerCore)                │
│   .zh.srt 出现/mtime 变 → MainActor.run {            │
│     player.loadExternalSubFile(url)                  │
│     // IINA 内部判：未加载 → mpv sub-add             │
│     //               已加载 → mpv sub-reload          │
│   }                                                  │
└──────────────────────────────────────────────────────┘
    │
    ↓ mpv sub-add / sub-reload command
[用户屏幕字幕实时更新]
```
...
| **API key 存储** | macOS Keychain（M4 起；通过 `AIKeychain` 包装 Security framework）| 拒绝 NSUserDefaults / Preference.swift（明文存敏感凭据）。M3 阶段临时存 `Preference.aiSubtitleDeepSeekKey`，M4 上线后做了一次性 migration → Keychain，pref 字段保留为空兜底 |
| **段级并发上限** | **1**（实证回退） | SPEC §5.3 原值 3。M3 跑通后实测 3 个并发 whisper-cli 与 mpv 同时抢 Metal GPU 触发音频 underrun（破音），首字幕延迟不变，故回退 1。M5 优化方向：whisper-cli 走 `-ng` 跑 CPU、或随用户暂停态动态调度 |
...
```
   ...
   AudioSegment       TranscriptionResult    [SrtLine]
                      ...
                      .zh.srt 文件
                            │
                            ↓
                  SubtitleFileWatcher (mtime 轮询 500ms · M5 换 FSEventStream)
                            │
                            ↓
                  PlayerCore.loadExternalSubFile(url) [IINA]
                  (内部按 subTracks 已含与否走 sub-add 或 sub-reload)
```
...
actor SRTMerger {
    ...
    /// - 重新编号（idx 连续）
    /// - 原子写（写临时文件再 rename，避免 watcher 看到半成品）
    func append(
        lines: [SrtLine],
        segmentIndex: Int,
...
> M3 决定先用 mtime 轮询替 FSEventStream（简单可靠，CPU 0%，无 atomic-rename 边界 case）。FSEventStream 留 M5 / V2 优化。
...
actor SubtitleFileWatcher {
    ...
    /// 启动 mtime 轮询（默认 500ms）
    /// 文件首次出现 → MainActor 上调 player.loadExternalSubFile(url) → mpv sub-add
    /// 后续 mtime 变化 → 同样调 loadExternalSubFile → IINA 内部走 sub-reload
    func start()

    /// 停止轮询。Task.cancel 让 sleep 抛 CancellationError 自然退出。
    func stop()
}
```
...
### 3.3 字幕重载（mtime 轮询 → mpv）

**好消息**：不需要改 IINA 既有代码。

实际选了 `PlayerCore.loadExternalSubFile(_:delay:)`（PlayerCore.swift:1508）而非 `reloadAllSubs()`——前者既能首次 sub-add，又能在 subTracks 已含时走 sub-reload 路径，不需要 watcher 区分"首次出现"和"后续修改"两种状态：
...
> M5 / V2 把轮询换成 FSEventStream 时这段不需要改。
...
**新建文件**：`luluk/PrefAISubtitleViewController.swift`（约 480 行 · M4 实装值，含程序化 UI 布局）

> **实装记**：M4 阶段决定**全程程序化构造 view**（无 xib），原因是 IINA 既有 PrefXxx 都是 xib，我们这一页结构靠 toggle/text field/popUpButton 动态 enable/disable，xib autoresize + binding 比代码堆复杂。Notification `.lulukAISubtitleConfigChanged` 串联"设置变更 → PlayerCore 失效旧 service → 下次 lazy 取 service 时按新 config snapshot 重建"。
...
**改动**：枚举 `Preference.Key` 末尾添加 9 个 key + 默认值（line 353-367 + line 1165-1173）：

```swift
// ▼ luluk: AI 字幕
static let aiSubtitleEnabled = Key("aiSubtitleEnabled")               // 默认 true
static let aiSubtitleProvider = Key("aiSubtitleProvider")             // "deepseek" / "minimax" / ...
static let aiSubtitleSourceLanguageMode = Key("aiSubtitleSourceLanguageMode")  // "auto" / "manual"
static let aiSubtitleManualSourceLanguage = Key("aiSubtitleManualSourceLanguage")  // "ja" / ...
static let aiSubtitleBilingual = Key("aiSubtitleBilingual")            // 默认 true
static let aiSubtitleAutoReload = Key("aiSubtitleAutoReload")          // 默认 true
static let aiSubtitleWhisperModel = Key("aiSubtitleWhisperModel")     // 默认 large-v3-turbo
static let aiSubtitleTokensUsed = Key("aiSubtitleTokensUsed")         // 累计 token 显示
static let aiSubtitleDeepSeekKey = Key("aiSubtitleDeepSeekKey")       // M3 临时入口；M4 上线后做了一次性 migration → Keychain，后续运行时此 key 不再读
// ▲ luluk 结束
```

> `aiSubtitleDeepSeekKey` 是 M3 在 UI 还没就绪时的"用 `defaults write` 设 key"通道；M4 启动时检测到非空就同步进 Keychain 并清空 pref 字段。保留这个 key 是为了让命令行调试路径不破。
...
- `PlayerCore.swift`：hook（openMainWindow 末尾，line ~645）+ cancel（stop()，line 1052）+ AISubtitleService 配置快照懒加载属性（line 232-265）+ Notification 监听（line 391-400）
- `PreferenceViewController.swift`：1 处（push tab）
- `Preference.swift`：1 处（9 个 key + 9 个默认值）
...
### M1：纯算法模块（Sanitizer + SRTMerger） — ✅ 已完成
...
### M2：进程 spawn 框架（AudioSplitter + WhisperRunner） — ✅ 已完成
...
### M3：单 provider 端到端（DeepSeek + 流水线） — ✅ 已完成

**目标**：跑通"视频 → 中文 SRT"完整流水线，但**只支持 DeepSeek 一个 provider**，UI 还没有。

**新建文件**（实际产出）：
- `luluk/AISubtitle/TranslationProvider.swift`（协议 + 公共数据结构 + `TranslationProviderConfig.batchSize/contextSize`）
...
- `luluk/AISubtitle/SubtitleFileWatcher.swift`（mtime 轮询，原计划 M5 提前到 M3 实装）
- `lulukTests/AISubtitleServiceTests.swift`（mock provider，跑流水线）

**集成 IINA**（§3.1 + §3.2 + §3.7）：
- patch `PlayerCore.swift`（hook 视频打开 + cancel）
- patch `Preference.swift`（注册 9 个 key + 默认值）
- 临时通过 `defaults write xyz.luluk.app aiSubtitleDeepSeekKey "sk-..."` 设 key（M4 上 UI + Keychain 后做 migration）

**实证调整**（vs §1.2 / §2.3 设计）：
- **段级并发 3 → 1**（M3 实测 3 并发 whisper-cli 与 mpv 抢 GPU 导致音频破音，commit `8b7d00b`）
- **SubtitleFileWatcher 用 mtime 轮询替 FSEventStream**（简化实现，避免 atomic-rename 边界 case；M5 真在意延迟再换）
- **PlayerCore.reloadAllSubs() → loadExternalSubFile(url)**（兼顾首次 sub-add + 后续 sub-reload）

**验证**（已通过）：
- 真实视频：打开 test.mp4，屏幕字幕在 ~11s 内自动出现中文
- 性能基线：turbo 模式实时 10× ≈ SPEC §7.6 表

**实际工时**：~~5-7 天~~ → 5 天（含若干 hotfix：commit `05f3018` ~ `c90d8ca` 加流水线诊断 print → NSLog；`468445b` whisper-cli PATH fallback；`df2e98f` AudioSplitter 默认关 silencedetect 防 3 分钟卡）

### M4：UI（设置面板 + 进度面板 + Keychain） — ✅ 已完成

**目标**：替换 M3 的命令行 `defaults write` 入口，让用户从 UI 配置 + 看进度。

**新建文件**（实际产出）：
- `luluk/AISubtitle/AIKeychain.swift`（Security framework 封装，存 DeepSeek API key）
- `luluk/PrefAISubtitleViewController.swift`（设置面板 ~480 行 · 4 sections · **全程程序化 UI**，无 xib）
- `luluk/AISubtitleProgressViewController.swift`（OSD 风格右上角悬浮进度面板）
- `lulukTests/AIKeychainTests.swift`（6 测试）

**集成 IINA**（§3.4 + §3.5）：
- patch `PreferenceViewController.swift`（push tab）
- `MainWindowController` 嵌入进度面板
- `PlayerCore` 加 `aiSubtitleService` 配置快照懒加载属性（snapshot 变 → 旧 service `cancel()` + 释放 → 下次访问按新 config 重建）
- `PlayerCore` 监听 `.lulukAISubtitleConfigChanged` Notification → `invalidateAISubtitleService()`
- M4 启动时一次性 migration：`Preference.aiSubtitleDeepSeekKey` 非空 → 写 Keychain + 清空 pref（命令行入口仍兼容）

**验证**（已通过）：
- 用户从 UI 切换 provider、填 API key（含三层保险：Enter / blur / 主动 save 按钮）
- 切换后下次打开视频立刻生效（reactive，不需要重启 app）
- 进度面板实时刷新（订阅 `AISubtitleService.progressStream`）

**实际工时**：~~4-6 天~~ → 1 天初版 + hotfix1-7 把 UI 布局错位 / Keychain 输入丢字 / Process 死锁 / 取消超时 / 0 字节 SRT 等问题逐一修掉（commit `1fee244` → `2ecbe37`）

**实装规模**（总计 117 单元测试，全绿）：
- M1: 26 (Sanitizer) + 19 (SrtLine) + 10 (SRTMerger) + 1 (基线) = 56
- M2: 9 (AudioSplitter) + 8 (WhisperRunner) + 7 (WhisperProcessPool) + 7 (ModelDownloader) = 31
- M3: 14 (DeepSeekProvider) + 10 (AISubtitleService) = 24
- M4: 6 (AIKeychain) = 6

### M5：FSEventStream + 多 provider + NLLB 兜底 — ⏳ 待启动
...
| M1 | 纯算法 + 测试 | 1-2 天 | ~1 天 | ✅ |
| M2 | 进程框架 | 3-5 天 | ~3 天 | ✅ |
| M3 | 单 provider 端到端（含 watcher 提前实装） | 5-7 天 | ~5 天 | ✅ |
| M4 | UI（程序化布局，无 xib）+ Keychain | 4-6 天 | ~1 天初版 + hotfix1-7 | ✅ |
| M5 | FSEventStream + 全 provider + NLLB | 6-8 天 | — | ⏳ |

M1-M4 实际工时明显短于预算，主要因为：(1) 原型 Python 代码已经把幻觉 / batch 大小 / prompt 等踩坑都验证过了，移植直翻；(2) M4 的 UI 选择程序化构造省去了 xib + autoresize 调试时间。M5 NLLB Python helper 打包预计仍是大头。
